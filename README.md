...
- `src/geometry/` — backend-independent mesh contracts plus the Wave Lamp surface generator, which builds a closed hollow wall with radial wave thickness. The mechanical assembly is explicitly `GenericBase -> RimInterface -> Decorative Shade`, with an optional centered `MountFeature`: `GenericBase` derives the compatible `BaseRimProfile`, `RimInterface` is generated from that profile, and decorative wave/twist/bulge stages never deform the base or rim. The base provides a broad support flange rather than acting as the mount tube.
